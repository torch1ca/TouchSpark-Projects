//------------------------------------------------------------------------------------------------------------------------
// 
// Title: SD Card Wav Player With Volume
//
// Description:
...
//    In this version I've added volume control, via a potentiometer 
...
//  I2S configuration
  ...
  uint32_t BytesReadSoFar=0;                   // Number of bytes read from file so far
  ...
  volatile int rotCounter = 25;
  int maxRot = 40;
  int minRot = 0;
  float maxVolume = 0.18f;

//------------------------------------------------------------------------------------------------------------------------

//Rotary encoder counter. Called in external pin interrupt
void EncoderValue() {
...
void loop()
{    
  int bytesAvailable = unoSerial.available();
  if (bytesAvailable > 0) {
    uint8_t* pData = (uint8_t *)malloc(1);
    unoSerial.readBytes(pData, 1);
    Serial.println(*pData);

    currentTrack = *pData;
    playingFile = true;
    BytesReadSoFar=0;
    LoadNewFile(paths[currentTrack]);
...
void LoadNewFile(const String& url){
  
  // get the wav file from the SD card
  WavFile = SD.open(url);                   // Open the wav file
  if(WavFile==false){
  ...
  else
  {
    ...
    // if the wav file contains a LIST section, skip past that to the data
    if (memcmp(&WavHeader.DataSectionID, "LIST", 4) == 0){
      WavFile.seek(44 + WavHeader.DataSize);
      uint8_t extendedHeader[8] = {};
      WavFile.read(extendedHeader, 8);
      memcpy(&WavHeader.DataSectionID, &extendedHeader[0], 4);
      memcpy(&WavHeader.DataSize, &extendedHeader[4], 4);
    }
...
uint16_t ReadFile(byte* Samples)
{
    ...
    //Volume = 0.04f;
    Volume = (float)rotCounter / (float)maxRot;
    Volume *= Volume;
    Volume *= maxVolume;

    // We now alter the data according to the volume control
    for(i=0;i<BytesToRead;i+=2)  // We step 2 bytes at a time as we're using 16bits per channel
    {         
      ...
      SignedSample=SignedSample*Volume;         // multiply by the volume - a value that will be between 0 and 1, 1 would be full vol    
